...
export const unocss = async (): Promise<Plugin[]> => {
  ...
  return [transform(uno, 'pre'), transform(uno, 'default'), preprocess(uno), virtual(uno), transform(uno, 'post')];
};
...
const transform = (uno: UnoGenerator, enforce: 'pre' | 'default' | 'post'): Plugin => {
  return {
