...
export const PointPurchase = builder.loadableObject('PointPurchase', {
  ...makeLoadableObjectFields(PointPurchases),
  fields: (t) => ({
    id: t.exposeID('id'),
    pointAmount: t.exposeInt('pointAmount'),
    paymentAmount: t.exposeInt('paymentAmount'),
    paymentMethod: t.expose('paymentMethod', { type: dbEnum(PaymentMethod) }),
    paymentData: t.expose('paymentData', { type: 'JSON' }),
    paymentResult: t.expose('paymentResult', { type: 'JSON', nullable: true }),
    state: t.expose('state', { type: dbEnum(PointPurchaseState) }),
    expiresAt: t.expose('expiresAt', { type: 'DateTime' }),
  }),
});

export const PointTransaction = builder.loadableObject('PointTransaction', {
  ...makeLoadableObjectFields(PointTransactions),
  fields: (t) => ({
    id: t.exposeID('id'),
    amount: t.exposeInt('amount'),
    cause: t.expose('cause', { type: dbEnum(PointTransactionCause) }),
...
const PurchasePointInput = builder.inputType('PurchasePointInput', {
  fields: (t) => ({
    paymentMethod: t.field({ type: dbEnum(PaymentMethod) }),
    pointAmount: t.int(),
