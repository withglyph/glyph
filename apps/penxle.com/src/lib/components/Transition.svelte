<script context="module" lang="ts">
  // source from: https://github.com/CaptainCodeman/svelte-transition
  ...
  function classes(classes: string) {
    return classes ? classes.split(' ').filter(Boolean) : [];
  }
...
<script lang="ts">
  ...
  function transition(node: HTMLElement, show: boolean | null) {
    ...
    function execute(show: boolean | null) {
      ...
      executing = run ? (show ? enter() : leave()) : Promise.resolve();
    ...
    if (parent) {
      parent.count++;
      // child updates happen here, as show properly is updated by the parent, which triggers the transition
