<script lang="ts">
  ...
  $: inFn = browser && window.innerWidth < 800 ? fly : fade;
</script>
        ...
        >
          ...
          <div>
            ...
            <div class={grid({ columns: 4, gap: '9px' })}>
              <ToggleButton checked={$data.pairs?.includes('BL')} on:change={(e) => checkPair(e, 'BL')}>
                BL
              </ToggleButton>
              <ToggleButton checked={$data.pairs?.includes('GL')} on:change={(e) => checkPair(e, 'GL')}>
                GL
              </ToggleButton>
              <ToggleButton checked={$data.pairs?.includes('HL')} on:change={(e) => checkPair(e, 'HL')}>
                HL
              </ToggleButton>
