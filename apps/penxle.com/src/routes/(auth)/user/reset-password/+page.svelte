...
<h1 class="font-bold text-xl w-full max-w-87.5 mb-6">새로운 비밀번호를 설정해 주세요</h1>
...
<Modal size="sm" bind:open>
  ...
  <Button slot="action" class="w-full" href="/login" size="xl" type="link">로그인하러가기</Button>
</Modal>

<Modal size="sm" bind:open={isExpired}>
  <svelte:fragment slot="title">링크가 만료되었어요</svelte:fragment>

  <Button slot="action" class="w-full" href="/user/reset-password" size="xl" type="link">계정찾기로 이동하기</Button>
</Modal>
