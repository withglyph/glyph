...
>
  ...
  >
    ...
    <Button style={css.raw({ width: 'full', hideFrom: 'sm' })} size="md" variant="brand-fill" on:click={insertLink}>
      삽입
    </Button>
    <Button style={css.raw({ width: '68px', hideBelow: 'sm' })} size="sm" variant="brand-fill" on:click={insertLink}>
      삽입
    </Button>
  </div>
</div>
