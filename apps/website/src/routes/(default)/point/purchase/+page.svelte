...
<form class={flex({ direction: 'column', gap: { base: '28px', sm: '48px' }, smDown: { paddingX: '20px' } })} use:form>
  <div
    class={flex({
      align: 'center',
      gap: '10px',
  ...
  <hr class={css({ border: 'none', height: '1px', backgroundColor: 'gray.50' })} />
  ...
  <div class={css({ width: 'full', maxWidth: '540px' })}>
    ...
    <Checkbox name="pointAgreement" size="sm">주문 내용 및 아래 유의 사항을 확인하였으며, 충전에 동의합니다.</Checkbox>

    <Button style={css.raw({ width: 'full' })} size="lg" type="submit" variant="gradation-fill">포인트 충전하기</Button>
  </div>
